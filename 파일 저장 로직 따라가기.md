...
```
지금 추측, 첨부 파일들 일단 서버에 저장은 되어있음. 

다시 메인 


```Java
logger.info("if(chgReq.getChumbus()!=null) 시작 , attachFiles 이 DTO에 담겨있음 ");

if(chgReq.getChumbus()!=null) {    
    List<UploadResponse> chumbsUploadResp = uploadPictures(chgReq);
