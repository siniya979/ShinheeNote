...
> 
> **<의존성 역전과 테스트 코드 >** 
> 외부 리소스를 사용하는 어떤 클래스의 테스트를 작성할 때, 의존성 역전이 아닌 의존성 주입이 되어 있다면 테스트 작성시에도 외부 리소스를 선언해야한다. 의존성이 직접 주입되어 있기 때문이다. 개발자는 어떤 외부 리소스를 의존하고 있는지 클래스 내부를 보지 않고는 알 수 없다. 따라서 테스트 작성을 위해 해당 클래스의 코드를 열어봐야 하고 이는 캡슐화가 지켜지지 않는 것이다. 
> 
> 또한 의존 하고 있는 외부 리소스에 문제가 있는 경우,  테스트 하려는 실제 클래스의 문제 여부와 상관 없이 테스트는 실패한다. 개발자는 외부 리소스의 로직을 검증하려는 것이 아니기 때문에 이런 상황은 잘못되었다. 
> 
> 이런 문제점을 해결하기 위해 의존성 역전을 사용한다. 의존성 역전 관계를 만들면 외부 리소스를 Mock 으로 대체할 수 있다. Mock 으로 대체한다는 것은 외부 리소스의 의존성 주입이 필요없다는 것을 의미하며 또한 외부 리소스의 로직이 잘못되었다 한 들 테스트 하고자 하는 클래스의 로직에는 영향을 주지 않는다.  더욱 명확한 테스트가 되는 것이다. 

> < final 메소드의 stub > 
> final 메소드는 Mock객체로 만들 수가 없다. final 키워드는 오버라이딩을 금지하기 때문이다. Mock 은 동적으로 클래스의 하위 클래스를 생성하여 메서드를 오버라이딩하고 원하는 동작을 제공하는 방식으로 mock 객체를 생성하는데, 오버라이딩이 불가능하니 mock 객체 생성이 안되는 것이다. 
> 
> 이 문제는 클래스 설계를 재검토하고 인터페이스의 추상화와 의존성 역전을 통해 해결할 수 있다. 
