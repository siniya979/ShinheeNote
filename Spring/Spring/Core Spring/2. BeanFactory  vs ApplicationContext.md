...
스프링 프레임워크는 가장 기본적이고 중요한 두 개의 패키지인 `org.springframework.beans` 와 `org.springframework.context`를 제공한다. 이 패키지의 코드는 스프링의 '**(IoC)제어의 역전**' 과 '**Dependency Injection(의존성 주입)**' 기능에 대한 기초를 제공한다. 